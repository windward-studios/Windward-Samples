...
import net.windward.datasource.mongodb.MongoDBDataSource;
...
public class GenerateReport extends HttpServlet {
	...
	public enum DatasourceType {
		...
		MySQL(Datatype.URL),
		Oracle(Datatype.URL),
		Cassandra(Datatype.URL),
		OData(Datatype.URL),
		Salesforce(Datatype.SALESFORCE),
		MongoDB(Datatype.URL),
		JSON(Datatype.FILE);
	 ...
	 * placed into service. Set the location of WindwardReports.properties here.  Also
	 * set the root path for Google App Engine VFS.
	 */
	...
	/**
	 * Called by the servlet container to indicate to a servlet that the servlet is being
	 * taken out of service.  Ensure Google App Engine VFS is closed
	 */
	public void destroy() {
		super.destroy();
		GaeVFS.close();
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		Map<String, String> map = null; // for variables if needed
			...
			try {
				...
				switch (templateData.datasourceType) {
					...
					case Salesforce:
                        ...
                        name = sfTemplateData.name;
						break;
					case MongoDB:
						url = "mongodb://" + urlTemplateData.username + ":"
								+ urlTemplateData.password + "@" + urlTemplateData.server
								+ "/" + urlTemplateData.database;
						context.log("Datasource: " + url);
						dsp = new MongoDBDataSource(url);
						break;
					case JSON:
						context.log("Datasource: " + fileTemplateData.datafile);
						dsp = new JsonDataSource(datafileStream);
						break;
					default:
						dsp = null;
				}
...
class FileDatasourceTemplate extends Template {
	...
	public FileDatasourceTemplate(String template, GenerateReport.DatasourceType datasourceType,
								  GenerateReport.Format format, String datafile, String name) {
...
class URLDatasourceTemplate extends Template {
	...
	public URLDatasourceTemplate(String template, GenerateReport.DatasourceType datasourceType,
								 ...
								 String server, String database, String name) {
		super(template, datasourceType, format);
...
class SalesforceTemplate extends Template {
	...
	public SalesforceTemplate(String template, GenerateReport.DatasourceType datasourceType,
							  GenerateReport.Format format, String username, String password, String token, String name) {
		super(template, datasourceType, format);
		this.username = username;
		this.password = password;
		this.token = token;
