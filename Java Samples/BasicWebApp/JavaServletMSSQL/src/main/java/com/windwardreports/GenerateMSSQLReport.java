...
public class GenerateMSSQLReport extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		// reminder to include the DB2 JDBC connector in the appserver lib
		try {
			Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver").newInstance();
		} catch (Exception e) {
			throw new ServletException("Please add the Sql Server JDBC connector to your classpath. Details at http://rpt.me/SqlServerJDBC", e);
		}
        ...
        try {
            ...
            report.processSetup();
            //setup our MSSQL datasource
            DataSourceProvider dsp = new JdbcDataSource("com.microsoft.sqlserver.jdbc.SQLServerDriver",
            					"jdbc:sqlserver://mssql.windward.net;DatabaseName=Northwind", "demo", "demo");
            report.processData(dsp, "MSSQL");
            dsp.close();
       } catch (Exception se) {
            log("Setup error", se);
